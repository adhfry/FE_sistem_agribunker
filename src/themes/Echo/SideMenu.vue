...
import AgriBunkerPNG from "@/assets/images/logo/agribunker.png";
import Cookies from "js-cookie";
import AhdaPhoto from "@/assets/images/team/ahda.jpg";
import ImamPhoto from "@/assets/images/team/imam.jpg";
import CindyPhoto from "@/assets/images/team/cindy.jpg";
import NoPhoto from "@/assets/images/team/nophoto.png";
import {
...
const logoutFunc = async () => {
  await postApi("/logout", null);
  Cookies.remove("session"); // Hapus cookie tertentu
  Object.keys(Cookies.get()).forEach((cookie) => Cookies.remove(cookie)); // Hapus semua cookies
  localStorage.clear();
  sessionStorage.clear();
  router.push("/auth/login");
};

const PhotoUser = ref(NoPhoto);

const user = ref("") as any;
const jabatanUser = computed(() => {
  let name = user.value ? user.value.name.split(" ")[0].toLowerCase() : "";
  let jabatan = "";
  if (name === "ahda") {
    jabatan = "Hacker";
  } else if (name === "imam") {
...
const getName = async () => {
  try {
    ...
    if (data && data.name) {
      const lowerName = data.name.toLowerCase(); // Konversi ke lowercase

      // Cek awalan nama
      if (lowerName.startsWith("ahda")) {
        PhotoUser.value = AhdaPhoto;
      } else if (lowerName.startsWith("imam")) {
        PhotoUser.value = ImamPhoto;
      } else if (lowerName.startsWith("cindy")) {
        PhotoUser.value = CindyPhoto;
      } else {
        PhotoUser.value = NoPhoto; // Jika tidak cocok, pakai foto default
      }
    }

    console.log(user.value);
    console.log(PhotoUser.value);
  } catch (error) {
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <div class="relative [&_div]:bg-white rounded-full">
                <img
                  :src="AgriBunkerPNG"
                  class="rounded-full"
                  alt="Logo AgriBunker"
          ...
          >
            ...
            <div class="flex items-center flex-1">
              ...
              <Menu class="ml-5">
                ...
                >
                  <img alt="User Photo" :src="PhotoUser" />
                </Menu.Button>
